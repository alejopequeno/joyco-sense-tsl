...
npm init -y
npm install three
npm install -D vite typescript @types/three vitest @types/node
```

`@types/node` is needed by `vite.config.ts`, which imports `node:url`.
...
`baseUrl` was removed in TypeScript 7, so `paths` entries must be relative —
hence the leading `./`. `types` pulls in `vite/client` for the `*.css`
side-effect import declaration and `node` for `vite.config.ts`.
